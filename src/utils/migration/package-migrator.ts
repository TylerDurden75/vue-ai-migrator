...
export async function migratePackageJson(
  ...
  try {
    ...
    if (allDeps.vuex) {
      ...
      // Add Pinia
      if (!packageJson.dependencies?.pinia && !packageJson.devDependencies?.pinia) {
    ...
    if (allDeps['@vue/test-utils'] && allDeps['@vue/test-utils'].startsWith('^1.')) {
      ...
      if (packageJson.devDependencies?.['@vue/test-utils']) {
        ...
        result.changes.push(`Vue Test Utils (dev): ${testUtilsVersion} → ^2.4.0`);
        result.modified = true;
      }
    }

    // Check for Vue 2 specific plugins that might need updates
    const vue2Plugins = [
    ...
    for (const plugin of vue2Plugins) {
      if (allDeps[plugin]) {
        result.warnings.push(
          `${plugin} is Vue 2 specific and may need to be replaced or updated`
        );
      }
    }

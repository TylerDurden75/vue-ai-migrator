...
program
  ...
  .description('Automatic Vue 2 → Vue 3 migration with codemods + AI combination')
  .version('0.5.0');
...
program
  ...
  .option('-p, --provider <provider>', 'AI provider: openai, mistral, claude, anthropic', 'openai')
  .option('-d, --dry-run', 'Dry run mode (does not modify files, shows diff)', false)
  .option('-o, --output <file>', 'Output file for the report', 'migration-report.json')
  .option('--no-ai', 'Disable AI usage')
  .option('--transformations <list>', 'List of transformations to apply (comma-separated)')
  .option('--no-rollback', 'Disable automatic backups')
  .option('--generate-tests', 'Generate Vitest tests for migrated components', false)
  .option('--show-diff', 'Show detailed diff for each file in dry-run mode', false)
  .option('--typescript', 'Enable TypeScript type annotations in migrated code', false)
  .action(async (path: string, options) => {
    ...
    try {
      ...
      const migrationOptions: MigrationOptions = {
        ...
        transformations: options.transformations ? options.transformations.split(',') : undefined,
      ...
      console.log(chalk.cyan(`  - Transformations applied: ${result.transformationsApplied}`));

      // Show classification summary if available
      if (result.classification) {
        console.log(chalk.cyan(`\n  Classification:`));
        console.log(chalk.green(`    🟢 Simple: ${result.classification.simple}`));
        console.log(chalk.yellow(`    🟡 Medium: ${result.classification.medium}`));
        console.log(chalk.red(`    🔴 Complex: ${result.classification.complex}`));
      }
...
program
  ...
  .option('--classify', 'Classify each file by complexity (simple/medium/complex)', false)
  ...
  .action(async (path: string, options) => {
    ...
    try {
      ...
      console.log(chalk.cyan(`  Vue version: ${analysis.vueVersion?.version || 'Not detected'}`));
      console.log(chalk.cyan(`  Vue files: ${analysis.vueFiles.length}`));
      console.log(chalk.cyan(`  Components found: ${analysis.componentsFound}`));
      console.log(chalk.cyan(`  Vue 2 patterns detected: ${analysis.vue2Patterns.length}`));
      ...
      if (options.classify && analysis.vueFiles.length > 0) {
        ...
        console.log(chalk.blue('\n📋 File Classification:'));
        const simple = classifications.filter((c) => c.classification.level === 'simple').length;
        const medium = classifications.filter((c) => c.classification.level === 'medium').length;
        const complex = classifications.filter((c) => c.classification.level === 'complex').length;
        ...
        if (classifications.length > 0) {
          ...
          for (const { file, classification } of classifications.slice(0, 5)) {
            ...
            console.log(chalk.cyan(`    ${emoji} ${fileName}: ${classification.level}`));
            if (classification.reasons.length > 0) {
              console.log(
                chalk.gray(`      Reasons: ${classification.reasons.slice(0, 2).join(', ')}`)
              );
            }
          }
        }
  ...
  .action(async (projectPath: string, options) => {
    ...
    try {
      ...
      if (backupCount === 0) {
        ...
        console.log(chalk.yellow('No backup files found. Nothing to rollback.'));
        process.exit(0);
      }
    ...
    try {
      ...
      if (options.format === 'console') {
        ...
        if (report.classification) {
          console.log(chalk.cyan(`\nClassification:`));
          console.log(chalk.green(`  🟢 Simple: ${report.classification.simple}`));
          console.log(chalk.yellow(`  🟡 Medium: ${report.classification.medium}`));
          console.log(chalk.red(`  🔴 Complex: ${report.classification.complex}`));
        }
        ...
        const mdPath = options.output.endsWith('.md') ? options.output : `${options.output}.md`;
        await fs.writeFile(mdPath, reporter['generateMarkdownReport'](report), 'utf-8');
...
program
  ...
  .option('-k, --ai-api-key <key>', 'API key for AI (or use OPENAI_API_KEY env variable)')
  .option('-o, --output <file>', 'Output file for the plan', 'migration-plan.json')
  .action(async (projectPath: string, options) => {
    const spinner = ora('Analyzing project and generating migration plan...').start();
    ...
    try {
      ...
      for (const item of sortedFiles.slice(0, 10)) {
        const priorityEmoji = item.priority >= 8 ? '🔴' : item.priority >= 5 ? '🟡' : '🟢';
        console.log(chalk.cyan(`  ${priorityEmoji} Priority ${item.priority}/10: ${item.file}`));
        console.log(chalk.gray(`     Reason: ${item.reason}`));
      }
      if (sortedFiles.length > 10) {
        console.log(chalk.gray(`  ... and ${sortedFiles.length - 10} more files`));
      }
